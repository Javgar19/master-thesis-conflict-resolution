...
def init_at_wp(radius, center, n_ac, waypoints_positions):
    ...
    for _ in range(n_ac):
        ...
        waypoints_positions.append(center)

        random_angle = random.random() * 360
        distance = radius 

        random_lat, random_lon = geo.qdrpos(center[0], center[1], random_angle, distance)
        dest_lat, dest_lon = geo.qdrpos(center[0], center[1], random.random() * 360, distance)

        heading = geo.qdrdist(random_lat, random_lon, center[0], center[1])[0] + 5*random.random()
        ...
        bs.traf.ap.route[idx].addwpt(iac=bs.traf.id.index(acid), name=acid, wptype=0,
                                                        lat=random_wp[0], lon=random_wp[1], alt=300, spd=random_speed)
        bs.traf.ap.route[idx].addwpt(iac=bs.traf.id.index(acid), name=acid + "_dest", wptype=0,
                                                        lat=dest_lat, lon=dest_lon, alt=300, spd=random_speed)                                                
        bs.traf.ap.setLNAV(idx=idx, flag=True)
                                            
        
    return ac_id


def create_waypoints(center, radius, n_waypoints=2):
	"""
...
def spawn_ac_wp(radius, center, number_of_aircrafts, waypoints_positions, ac_id):
    ...
    for _ in range(number_of_aircrafts):
        ...
        random_wp = random.choice(waypoints_positions)
        heading = geo.qdrdist(random_lat, random_lon, dest_lat, dest_lon)[0]
         
        acid = str(ac_id)
        ac_id += 1

        bs.traf.cre(acid, actype="M200", aclat=random_lat, aclon=random_lon, acalt=300, acspd=random_speed)
        idx = bs.traf.id.index(acid)
...
def plot_at(center, radius, sources_position):
    ...
    plt.quiver(x, y, vy, vx, label="Aircraft")
    plt.scatter(center[0], center[1], 18, label="Circle center")
    plt.scatter([coord[0] for coord in coords], [coord[1] for coord in coords], 2, label="Simulation boundaries")
    plt.scatter([coord[0] for coord in sources_position], [coord[1] for coord in sources_position], 18, label="Waypoint positions")
    plt.title("Scenario initial configuration", fontsize=12)
...
def complexity_simulation(_run, center, radius, n_ac, sim_time, n_runs, rpz, tcpa_thresh):
    ...
    for run in range(n_runs):
        ...
        while bs.sim.simt < sim_time:
            ...
            comp_confs = ind.comp_conf(conf_graph)
            print(f"Conflicts: {len(bs.traf.cd.confpairs_unique)}")
            if (len(comp_confs)):
